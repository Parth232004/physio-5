...
class NeuroSafeSignalEngine:
    ...
    def _detect_phase(
        ...
        if angles:
            ...
            if is_moving:
                ...
                avg_flexion = sum(
                    angles.get(k, 0) for k in angles 
                    if 'shoulder_flexion' in k
                ) / max(1, sum(1 for k in angles if 'shoulder_flexion' in k))
    ...
    def _format_unreal(self, signal: SafetyFrameSignal) -> Dict:
        ...
        return {
            ...
            "display_text": self._get_display_text(signal),
            "phase": signal.phase,
            "frame": signal.frame_number,
    ...
    def _format_vr(self, signal: SafetyFrameSignal) -> Dict:
        ...
        return {
            "haptic_intensity": self._get_haptic(signal.severity),
            "audio_cue": self._get_audio(signal.safety_flag),
            "visual_color": self._get_visual(signal.safety_flag),
            "safety_flag": signal.safety_flag,
            "confidence": signal.confidence,
            "correction": signal.correction,
    ...
    def _format_minimal(self, signal: SafetyFrameSignal) -> Dict:
        """Minimal format for high-performance systems"""
        return {
            "s": signal.safety_flag[0].upper(),  # S, W, D, U
            "c": round(signal.confidence, 2),
            "v": signal.severity,
            "p": signal.phase[0].upper(),  # R, I, A, T, C, U
            "n": 1 if signal.is_new else 0,
            "t": round(signal.timestamp, 3)
        }
    
    def _get_unreal_command(self, signal: SafetyFrameSignal) -> str:
        """Get Unreal Engine command code"""
        if signal.severity >= 3:
            return "STOP_IMMEDIATELY"
        elif signal.severity >= 2:
            return "CORRECT_POSITION"
        elif signal.severity >= 1:
            return "MONITOR_CLOSELY"
        else:
            return "CONTINUE"
    
    def _get_display_text(self, signal: SafetyFrameSignal) -> str:
        """Get human-readable display text"""
        if signal.safety_flag == "danger":
            return "✗ STOP - Dangerous position!"
        elif signal.safety_flag == "warning":
            if signal.correction:
                return f"⚠ {signal.correction['instruction']}"
            return "⚠ Position needs correction"
        elif signal.safety_flag == "safe":
            return "✓ Position is correct"
        else:
            return "? Unable to assess"
    
    def _get_haptic(self, severity: int) -> int:
        """Get haptic feedback intensity (0-255)"""
        haptic_map = {0: 0, 1: 50, 2: 150, 3: 255}
        return haptic_map.get(severity, 0)
    
    def _get_audio(self, safety_flag: str) -> str:
        """Get audio cue identifier"""
        audio_map = {
            "safe": "success_chime",
            "warning": "warning_tone",
            "danger": "emergency_alarm",
            "unknown": "tracking_lost"
        }
        return audio_map.get(safety_flag, "none")
    
    def _get_visual(self, safety_flag: str) -> str:
        ...
        return visual_map.get(safety_flag, "gray")


# Backward compatibility class
class SignalGenerator(NeuroSafeSignalEngine):
    """
    Backward-compatible SignalGenerator wrapper.
    Wraps the new NeuroSafeSignalEngine.
    """
    
    def __init__(self, suppress_repeated: bool = True):
        super().__init__(
            cooldown_enabled=True,
            deduplication_enabled=suppress_repeated
        )
    
    def generate_signals(self, assessment: SafetyAssessment) -> Dict:
        """Generate signals (backward compatible method)"""
        frame_signal = self.process_frame(assessment)
        return {
            "json": frame_signal.to_json(),
            "unreal": self._format_unreal(frame_signal),
            "vr": self._format_vr(frame_signal),
            "minimal": self._format_minimal(frame_signal)
        }


class VRMessageFormatter:
    """Backward-compatible VR message formatter"""
    
    @staticmethod
    def format_for_unreal(signal_batch: Dict) -> Dict:
        """Format signals for Unreal Engine"""
        return signal_batch.get("unreal", {})
    
    @staticmethod
    def format_for_vr(signal_batch: Dict) -> Dict:
        """Format signals for VR systems"""
        return signal_batch.get("vr", {})
    
    @staticmethod
    def format_minimal(signal_batch: Dict) -> Dict:
        """Minimal format"""
        return signal_batch.get("minimal", {})
