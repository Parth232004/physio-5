...
class SafetyRules:
    ...
    def __init__(self, config_path: str = "config.json"):
        ...
        self.thresholds = self._build_thresholds(config['safety_thresholds'])
        self.min_confidence = config['confidence']['min_threshold']
        self.suppress_repeated = config['output']['suppress_repeated_warnings']
        ...
        for movement, values in threshold_config['shoulder'].items():
            for side in ['left', 'right']:
                ...
                thresholds[key] = SafetyThreshold(
                    ...
                    safe_max=values['safe_max'],
                    warning_max=values['warning_max'],
                    danger_max=values['safe_max'] * 1.1,
                    unit=values['unit']
                )
        ...
        for movement, values in threshold_config['elbow'].items():
            for side in ['left', 'right']:
                ...
                thresholds[key] = SafetyThreshold(
                    ...
                    safe_max=values['safe_max'],
                    warning_max=values['warning_max'],
                    danger_max=values.get('danger_threshold', values['safe_max'] * 1.1),
                    unit=values['unit']
                )
            ...
            if len(parts) >= 3:
                ...
                if level == SafetyLevel.DANGER:
                    violations.append(SafetyViolation(
                        ...
                        safe_limit=self.get_threshold(joint, movement).danger_max,
                        safety_level=level,
                        message=message,
                        timestamp=timestamp
                    ))
                    ...
                    violations.append(SafetyViolation(
                        ...
                        movement=movement,
                        current_angle=angle_value,
                        safe_limit=self.get_threshold(joint, movement).warning_max,
                        safety_level=level,
                        message=message,
                        timestamp=timestamp
                    ))
